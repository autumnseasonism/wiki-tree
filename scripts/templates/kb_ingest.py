...
DEFAULT_POLICY = {
    ...
    "skill_scripts_dir": "",  # wiki-tree/scripts 目录；留空自动探测
}
...
def resolve_scripts_dir(pol):
    cands = [pol.get("skill_scripts_dir") or "",
             str(HOME / ".claude" / "skills" / "wiki-tree" / "scripts"),
             str(HOME / ".codex" / "skills" / "wiki-tree" / "scripts")]
    for c in cands:
...
def print_handoff(by_kb, scripts, built):
    sd = scripts or "<wiki-tree/scripts>"
...
def main():
    ...
    if a.finalize:
        ...
        if not root or not Path(root).exists():
            ...
            sys.exit(2)
        if not scripts:
            print("错误：找不到 wiki-tree/scripts；请在 ingest-policy.json 设 skill_scripts_dir",
                  file=sys.stderr)
            sys.exit(2)
        try:
