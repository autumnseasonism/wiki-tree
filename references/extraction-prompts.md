...
重要性评分标准（文档口径）：
- 0.9+：决策记录、规格/承诺与截止期、可执行结论
- 0.6+：实质性事实与数据、命名实体密集的讨论
- 0.3+：背景综述、低密度散文
- < 0.3：目录页、模板残留、无信息量的导出物

topics 给 1-3 个粗粒度领域词（2-6 字），优先复用通用领域名（如「项目管理」「AI 工程」），不自创长短语。
```
...
严格返回 JSON，不要输出 JSON 以外任何内容（包括代码围栏）。
```
...
硬约束：subject 和 object 必须逐字使用实体列表中的 text，禁止使用列表外的名称、别名或改写；找不到匹配实体的关系直接丢弃。
...
### 落盘骨架（`summaries/topic-{规范主题名}.md`）

文件名清洗：`\ / : * ? " < > |` 与换行/制表符替换为 `-`（与脚本 `_safe` 规则一致）。**正文第一行必须是 `**一句话**：` 行**——`emit_access_bundle.py` 据此提取 `one_liner`，检索与入库路由都用它：
...
## 时间线
- {date}：{event}（来源：{source}）

## 关键实体
[[person-张三]]、[[concept-RAG]]（key_entities 中**仅链接已建卡实体**，未建卡的写普通文本，避免悬空 wikilink）
```
