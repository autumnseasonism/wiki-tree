...
**增量模式（已有 Vault 时复跑）**：若目标 Vault 已存在，给 `scan_folder.py` 传 `--vault {vault}`，它会读取 `{vault}/.wiki-tree/manifest.json`，为每个文件标记 `new / modified / done`：
...
- `generate_wiki_structure.py` 默认**缺失才创建**模板（`_index.md`/`_global-summary.md`/`_knowledge-graph.md`），重跑不覆盖 Agent 已填内容；需重置版式时显式加 `--force`。它还会创建 `.wiki-tree/`（manifest + `extracted/`）这一增量/并行共享底座。
...
每篇文档抽取完成后，把实体/关系/摘要写入 `{vault}/.wiki-tree/extracted/{doc-id}.json`（schema 见 `references/subagent-batch-extraction.md`）。`extracted/` 是持久缓存，reduce 永远从其全集重建图谱（幂等）。
...
- **fan-in（主 agent 独揽登记）**：收齐所有 worker 的 `done` 清单 → 合并成一个 JSON 数组写到 `{vault}/.wiki-tree/_marks.json` → 调一次 `python scripts/update_manifest.py --vault {vault} --mark-from {vault}/.wiki-tree/_marks.json` 串行登记。**单一写入者 → 无并发写竞态。**
...
**输入 = `{vault}/.wiki-tree/extracted/*.json` 全集**（本轮 + 历轮累积，增量复跑据此重建，幂等）。汇总所有文档的实体和关系，构建知识图谱：

1. **实体去重（语义，交 LLM）**：合并同一实体的不同写法（如"张三/Zhang San"）。可先跑 `python scripts/suggest_dedup.py --vault {vault}` 拿到**确定性候选变体对**（归一化相等 / 子串 / 编辑距离≤1）作为线索，再由你按语义确认；把最终"变体→规范名"映射写成 `{vault}/.wiki-tree/_dedup-map.json` 供下一步复用。（跨语言译名等无字面相似的变体仍需你凭语义补充——脚本只给线索、不下结论。）
...
**四档下钻**（`kb_query.py` 内置，无需额外文件）：`--global`(L2 全局) → 主题摘要(L1，检索结果或 `--topic`) → `--level detailed`(逐文档详细摘要，取自 `.wiki-tree/extracted/`) → `--level full`(L0 原文)。检索默认 `--level short`，agent 拿到主题摘要+候选文档路径后**按需下钻、够用就停**。
...
1. `scan_folder.py --vault {vault}` 读取 `.wiki-tree/manifest.json`，逐文件判定：
   - 不在 manifest → `new`（待处理）
...
- [ ] （增量）确认被 >100 推迟的文件在下一轮被补上；（并行）确认子 agent 都写入了 `.wiki-tree/extracted/` 且 manifest 已登记
